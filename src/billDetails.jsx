  ...
  return (
    <div style={{}}>
      <div style={{display: "flex", justifyContent: "center", alignItems: "center"}}>
        <h1 style={{paddingRight: "150px"}}>{bill.title}</h1>
      ...
      {explanation && (
        <div>
          <h2>Explanation</h2>
          <p style={{ alignItems: "center", background: "black", color: "white", padding: "20px", width: "500px"}}>{explanation}</p>
        </div>
      )}
      <div style={{display: "flex", alignItems: "center", gap: "20px"}}>
        <button style={{ background: "green" }} onClick={handleUpvote}>Upvote</button>
        <span>{bill.upvotes}</span>
        <button style={{ background: "red" }} onClick={handleDownvote}>Downvote</button>
        <span>{bill.downvotes}</span>
      </div>
      <div>
        <h2>Comments</h2>
